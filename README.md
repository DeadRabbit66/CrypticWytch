...
> ⚠️ **Внимание!** Ключ сеансовый и теряется после закрытия программы.  
> Сохраните его отдельно, если нужно повторно расшифровать данные позже.
> Для каждого нового сеанса генерируется новый ключ
...
Для максимальной скрытности вы можете встроить зашифрованный текст в изображение с помощью [Image Rat](https://github.com/DeadRabbit66/ImageRat).

Схема работы:
1. Шифруете текст в **Cryptic Wytch**
2. Прячете результат в картинку через **Image Rat**
3. Отправляете картинку!

> 🐀 Image Rat — простая стеганография через LSB.
